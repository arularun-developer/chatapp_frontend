...
const MyChat = ({ myChats, dispatch }) => {
    ...
    return (
        <Fragment>
            <div className="flex flex-col border-r bg-neutral-900 w-full h-[89.5vh]">
                <div className="flex justify-between items-center  w-full h-[60px] px-4 py-2 border-b">
                    <div className="flex items-center space-x-8 ms-auto ">
                        <svg onClick={() => dispatch({ type: 'SHOWGUS' })} width="40" height="40" className="cursor-pointer" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" clip-rule="evenodd" viewBox="0 0 48 48" id="group">
                ...
                <div className="flex space-x-2 items-center w-full h-[60px] py-2 bg-zinc-900 border-0   my-2 rounded-lg">
                    <div className="flex-grow flex items-center space-x-2 py-1 px-4 rounded-md">
                        <svg viewBox="0 0 24 24" width="40" height="40" className="cursor-pointer">
                            <path fill="#808080"
                                d="M15.009 13.805h-.636l-.22-.219a5.184 5.184 0 0 0 1.256-3.386 5.207 5.207 0 1 0-5.207 5.208 5.183 5.183 0 0 0 3.385-1.255l.221.22v.635l4.004 3.999 1.194-1.195-3.997-4.007zm-4.808 0a3.605 3.605 0 1 1 0-7.21 3.605 3.605 0 0 1 0 7.21z">
                            </path>
                        </svg>
                        <input onKeyUp={TextSearch} type="search" className="focus:outline-none w-full text-white  h-10 bg-zinc-900 text-md rounded-lg p-2"
                ...
                <div className="flex flex-col overflow-y-auto cursor-pointer w-full h-full ">
                    {
                        myChats?.map((items, i) => {
                            return (
                                <button key={i} onClick={accessChatMsg.bind(this, items)} className={`flex items-center w-full px-4 py-2 my-2 transition-colors duration-200 gap-x-3 ${selectUser?._id === items._id ? "bg-neutral-800 " : "hover:bg-black"} focus:outline-none`}>
                                    <div className="relative w-14">
                                        <img
                                            className={`${items.isGroupChat && 'bg-gray-700'} object-cover w-12 h-12 rounded-full`}
...

                                        {
                                            ...
                                            <span className="h-3 w-3 rounded-full  absolute right-0.5 ring-2 ring-white -bottom-0.5"></span>
                                        }
                                    </div>
                                    <div className='flex justify-between w-full'>
                                        <div className="text-left rtl:text-right space-y-1 ">
                                            <h1 id='ChatName' className="text-sm font-medium text-white capitalize">
                                                {items.isGroupChat ?
                                                    items.chatName : getSender(items.users, getUserDetails()).firstname + ' ' + getSender(items.users, getUserDetails()).lastname
                                                }
                                            </h1>
                                            <div className={`${notifications.find((n) => n.chat._id === items._id) ? 'text-[#808080]' : 'text-white'} flex gap-2 items-center w-full`}>
                                                {
                                        ...
                                        <div className="flex flex-col flex-none text-xs items-end space-y-2 w-20 pr-2">
                                            <p className={`${notifications.find((n) => n.chat._id === items._id) ? 'text-[#808080] font-medium' : 'text-white'} text-xs`}>{moment(items.latestMessage?.createdAt).format("DD/MM/YYYY")}</p>
